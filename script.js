dots = {
  init: function() {
    ...
    var data = [],
        axisx = [],
        axisy = [],
        $table = jQuery("#changes");
    jQuery("tbody td", $table).each(function (i) {
    ...
    jQuery("tfoot th", $table).each(function () {
    ...
    var width = 400,
        ...
        bottomgutter = 20,
        midpadding = 20,
        r = Raphael("changes-chart", width, height),
        txt = {"font": '15px Georgia, serif', stroke: "none", fill: "#000"},
        X = (width - leftgutter) / axisx.length,
        Y = (height - bottomgutter) / axisy.length,
        color = jQuery("#changes-chart").css("color"),
        max = Math.round(X / 2) - 1;
    // r.rect(0, 0, width, height, 5).attr({fill: "#000", stroke: "none"});
    for (var i = 0, ii = axisx.length; i < ii; i++) {
      r.text(X * (i + .5), height-6, axisx[i]).attr(txt);
    }
    for (var i = 0, ii = axisy.length; i < ii; i++) {
      page = r.text(width - leftgutter + midpadding, Y * (i + .5), axisy[i]).attr(txt);
      page.attr('href', axisy[i]);
      page.attr('fill', '#08c');
      page.attr('text-anchor', 'start');
    }
    var o = 0;
    for (var i = 0, ii = axisy.length; i < ii; i++) {
